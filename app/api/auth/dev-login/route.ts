...
export async function GET(request: Request) {
  ...
  const testEmail = 'dxiaochuan@gmail.com'
  ...
  if (error || !data.properties?.hashed_token) {
    return NextResponse.json({ error: error?.message || 'Failed to generate session' }, { status: 400 })
  }
  ...
  if (verifyError || !sessionData.session) {
    return NextResponse.json({ error: verifyError?.message || 'Failed to create session' }, { status: 400 })
  }
